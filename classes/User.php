...
class User {
...

	{
...

		{
			...
			$auth=$_POST['auth'];
			$username = $this->mysqli->real_escape_string(strip_tags(trim($username)));
      		  	$password = md5(addslashes(strip_tags(trim($password)))); 
...

	{
		...
		if ($newPassword != $confirmPassword)
		{
			...
			{
				$user = $result->fetch_assoc();
				$oldPassword = md5(addslashes(strip_tags(trim($oldPassword))));
				$newPassword = md5(addslashes(strip_tags(trim($newPassword))));
	...
	{
		global $hide_ids;
        	$password = md5(addslashes(strip_tags(trim($newPassword))));
		if (is_numeric($id) && !in_array($id, $hide_ids))
		{
